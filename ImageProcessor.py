# modified version of code found in https://github.com/neeru1207/AI_Sudoku/blob/master/BoardExtractor.py
import cv2
import numpy as np
...
class ImageProcessor:
    ...
    def detect_and_crop_grid(self):
        ...
        for y in range(height):
            ...
            for x in range(width):
                if row[x] >= 128: # if pixel is black
        ...
        # Calculate the expected size of the outer grid (Sudoku board)
        number_of_rows, number_of_columns = 9, 9
        cell_size = min(height, width) // number_of_rows
        expected_size = (number_of_rows * cell_size, number_of_columns * cell_size)
        # Calculate the size difference between the found blob and the expected size
        size_difference = abs(maxi - (expected_size[0] * expected_size[1]))
        # Define a threshold to determine if the board is not found
        threshold = 100000

        if size_difference < threshold:
            raise Exception(f" Sudoku board not found. Please check the input image.")
    ...
    of cell images, and also pre processes every 28x28 image to remove unwanted lines'''
    def create_image_grid(self):
        def postProcessCell(cell):
            img = cell
            rows = np.shape(img)[0]
        ...
        for i in range(9):
            for j in range(9):
                orgCell = np.array(finalgrid[i][j])
                newCell = postProcessCell(orgCell)
                finalgrid[i][j] = newCell
